	...
	$( ".draggable" ).draggable({
		create: function(){
            ...
            var draggableObject = {
				...
				currentDropPos: "",
				correctDropPos: classRef._answersData.find( "drag:contains("+$(this).attr("id")+")" ).parent().children( "drop" ).text()
			};
...
ERS.DragAndDrop.prototype._checkAnswers = function() {
	for( var i = 0; i < this._draggableItems.length; i++ )
	{
		// if this draggable item's current drop pos is not correct, return false
		if( this._draggableItems[i].currentDropPos != this._draggableItems[i].correctDropPos )
			return false;
	}
...
ERS.DragAndDrop.prototype._displayAnswers = function() {
	...
	for( var i = 0; i < this._draggableItems.length; i++ )
	{
		var endX = $( "#" + this._draggableItems[i].correctDropPos + ".dropPos" ).position().left;
		var endY = $( "#" + this._draggableItems[i].correctDropPos + ".dropPos" ).position().top;
